...
def fresh(*workers: str) -> Broker:
    b = Broker()
    for w in workers:
        call(b, "announce", handle=w, model="sonnet", effort="medium")
    return b
...
def main() -> int:
    ...
    call(b, "announce", handle="w2", model="sonnet", effort="medium")
    ...
    check("an unclaimed worker is open to anyone",
          call(b, "announce", handle="w3", model="haiku", effort="low").get("ok")
    ...
    call(b, "claim", handle="w1", owner="dispA")
    call(b, "announce", handle="dispA", model="opus", effort="high")
